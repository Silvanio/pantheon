...
- [x] 7.4 Run full stack via `infra/docker-compose.yml`, invite an unregistered email, confirm the email appears in Mailpit, complete registration + accept, confirm the badge disappears and the member gains access — performed live during `restructure-company-obra-hierarchy`'s manual QA (invitations now target a construction site/obra rather than a project, per that change's generalized `membershipType`), confirming the underlying invitation lifecycle end-to-end.
- [x] 7.5 Superseded by `restructure-company-obra-hierarchy`, which generalized `MembershipInvitation` (`membershipType` COMPANY/SITE) on top of this change's tables/lifecycle; archiving both together.
